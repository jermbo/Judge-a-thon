  ...
  try {
    ...
    Teams.findById(TeamId, (err, team) => {
      if (team.Judgments) {
        ...
        Teams.update(
          { _id: TeamId },
          { $set: { Judgments: judgments, TotalScore: TotalScore } },
          (err, UpdatedTeam) => {
            noOfJugments++;
            ...
            if (noOfJugments >= NoOfJudges) {
              ...
              Teams.find({ HackathonId: HackathonId }).then(teams => {
                ...
                console.log(
                  "We have these many teams left to be judged by this judge:" +
                    TeamsLeftToJudge.length,
                );
                  ...
                  Teams.find({ HackathonId: HackathonId })
                    ...
                    .then(teams => {
                      ...
                      teams.map(t => {
                        ...
                        PlacementUpdatePromises.push(
                          Teams.update(
                            { _id: id },
                            { $set: { RankText: RankText } },
                          ),
                        );
                        counter++;
                      });
